...
export function HomePageClient() {
  ...
  return (
    <div>
      ...
      <section className="py-24 px-4 bg-luxury-rich border-y border-gold-matte/10">
        <div className="max-w-6xl mx-auto">
          <SectionHeader label="آراء عملائنا" title="ثقة تليق بضيوفنا" />
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { name: "أبو فيصل", event: "حفل زفاف - جدة", text: "خدمة احترافية من أول الاتصال لآخر لحظة. القهوجية والصبابين بزي فاخر وأداء يفوق التوقعات. ضيوفي انبهروا.", rating: 5 },
              { name: "منظّمة فعالية", event: "مؤتمر شركات - الرياض", text: "تعاملنا مع كيف الضيافة في مؤتمرنا السنوي. التنظيم دقيق، التقديمات فاخرة، وفريق منظم. تجربة تستحق التكرار.", rating: 5 },
              { name: "أم عبدالله", event: "حفل تخرج - الطائف", text: "الطاقم النسائي (المباشرات) محترمات ومنظّمات. القهوة والتمر والتقديمات كلها بمعايير عالية. شكراً كيف الضيافة.", rating: 5 },
            ].map((t, i) => (
              <motion.div
                key={i}
                initial={{ opacity: 0, y: 20 }}
                ...
                className="card-luxury p-8 rounded-3xl"
              >
                ...
                <div className="flex items-center gap-3 pt-4 border-t border-[#B8860B]/10">
                  ...
                  <div>
                    <p className="text-[#F5F5DC] text-sm" style={{ fontWeight: 600 }}>{t.name}</p>
                    <p className="text-[#F5F5DC]/40 text-xs">{t.event}</p>
                  </div>
                </div>
              </motion.div>
            ))}
          ...
          <div className="text-center mt-10">
            <p className="text-[#F5F5DC]/40 text-xs">+500 مناسبة ناجحة · +200 عميل · 100% رضا · منذ 2016</p>
          </div>
        </div>
      </section>
    </div>
