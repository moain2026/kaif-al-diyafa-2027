...
💬 ${formData.message}`;
  ...
  return (
    <div>
      ...
      {/* FORM — Progressive enhancement: visible by default, JS enhances with animation */}
      <section className="px-4 pb-20">
        <div className="max-w-2xl mx-auto">
          <form onSubmit={handleSubmit} className="contact-form p-6 sm:p-8 rounded-3xl" style={{ background: "linear-gradient(160deg, rgba(25,20,8,0.9), rgba(15,12,5,0.95))", border: "1px solid rgba(184,134,11,0.2)" }}>
            <h2 className="text-[#F5F5DC] mb-6" style={{ fontSize: "1.3rem", fontWeight: 700}}>أرسل لنا رسالتك</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
              <div>
                <label htmlFor="contact-name" className="block text-[#F5F5DC]/50 text-xs mb-2">الاسم *</label>
                <input id="contact-name" name="name" type="text" required autoComplete="name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full px-4 py-3 rounded-xl bg-[#0f0f0f] text-[#F5F5DC] text-sm placeholder-[#F5F5DC]/25" style={{ border: "1px solid rgba(184,134,11,0.15)" }} placeholder="اسمك الكريم" />
              </div>
              <div>
                <label htmlFor="contact-phone" className="block text-[#F5F5DC]/50 text-xs mb-2">رقم الجوال *</label>
                <input id="contact-phone" name="phone" type="tel" required pattern="^(\+966|0)?5[0-9]{8}$" maxLength={13} autoComplete="tel" title="رقم جوال سعودي صحيح (مثال: 05XXXXXXXX أو +9665XXXXXXXX)" value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} className="w-full px-4 py-3 rounded-xl bg-[#0f0f0f] text-[#F5F5DC] text-sm placeholder-[#F5F5DC]/25" style={{ border: "1px solid rgba(184,134,11,0.15)" }} placeholder="+9665XXXXXXXX" dir="ltr" />
              </div>
              <div>
                <label htmlFor="contact-email" className="block text-[#F5F5DC]/50 text-xs mb-2">البريد الإلكتروني</label>
            ...
            <div className="mb-4">
              <label htmlFor="contact-service" className="block text-[#F5F5DC]/50 text-xs mb-2">نوع الخدمة</label>
              <select id="contact-service" name="service" value={formData.service} onChange={(e) => setFormData({ ...formData, service: e.target.value })} className="w-full px-4 py-3 rounded-xl bg-[#0f0f0f] text-[#F5F5DC] text-sm" style={{ border: "1px solid rgba(184,134,11,0.15)" }}>
            ...
            <div className="mb-6">
              <label htmlFor="contact-message" className="block text-[#F5F5DC]/50 text-xs mb-2">رسالتك *</label>
              <textarea id="contact-message" name="message" required rows={4} value={formData.message} onChange={(e) => setFormData({ ...formData, message: e.target.value })} className="w-full px-4 py-3 rounded-xl bg-[#0f0f0f] text-[#F5F5DC] text-sm placeholder-[#F5F5DC]/25 resize-none" style={{ border: "1px solid rgba(184,134,11,0.15)" }} placeholder="اكتب تفاصيل مناسبتك هنا..." />
            </div>
            <button type="submit" className="contact-submit-btn w-full flex items-center justify-center gap-3 py-4 rounded-full text-white transition-transform duration-200 hover:scale-[1.02] active:scale-[0.98]" style={{ background: "linear-gradient(135deg, #1da851, #25D366)", fontWeight: 700, fontSize: "1rem", boxShadow: "0 6px 25px rgba(37,211,102,0.35)" }}>
