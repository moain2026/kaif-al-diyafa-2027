    ...
    <>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 md:hidden safe-bottom bottom-nav" style={{ paddingBottom: "max(12px, env(safe-area-inset-bottom))" }}>
        <div className="flex items-center justify-between px-4 pt-3 pb-1">
          ...
          <motion.button onClick={toggleMenu} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.93 }} className="flex items-center gap-2 px-5 py-2.5 rounded-full text-luxury-black min-h-[44px]" style={{ background: "var(--gradient-gold-matte)", fontWeight: 800, fontSize: "0.85rem", boxShadow: "0 4px 20px rgba(197,160,89,0.4)" }} aria-label={menuOpen ? "إغلاق القائمة" : "فتح القائمة"} aria-expanded={menuOpen} aria-controls="mobile-nav-menu">
